        ...
        if(result.isPresent()) {
        	...
        	try {
        		...
        		if(period.getYears() >= 18) {
        			...
        			update = new updateUserData("Idade", String.valueOf(period.getYears()), Wallet.Home.getId());
                	birthday.setText(result.get());
                	Age.setText(String.valueOf(period.getYears()));
        		}else {
        			Alert alert = new Alert(AlertType.WARNING);
            		alert.setTitle("Warning");
            		alert.setContentText("you are under 18");
        ...
        if(textMatches == false) {
			if(!Amount.getText().isEmpty() && Float.parseFloat(Amount.getText()) > 0) {
				if(Float.parseFloat(Amount.getText()) <= Float.parseFloat(betCoins.getText())) {
					int node = Integer.parseInt(Main.getLinkBlockchain().substring(Main.getLinkBlockchain().length() - 4));
					sendCoins transfer = new sendCoins(strpublicAddr, strprivateAddr, Address.getText(), Float.parseFloat(Amount.getText()), Integer.toString(Wallet.Home.getId()), node);
