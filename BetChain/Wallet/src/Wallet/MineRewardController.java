...
public class MineRewardController implements Initializable{	
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		Label label1 = new Label("No transactions to be validated");
		label1.setTextFill(Color.WHITE);
		Label label2  = new Label("No validated transactions");
		label2.setTextFill(Color.WHITE);
		
		tableNotValidated.setPlaceholder(label1);
		tableValidated.setPlaceholder(label2);
		
		getTransactionTable getTable = new getTransactionTable();
		if(getTable.getSenderList().size() > 0) {
	...
	public void mineration(ActionEvent ev) throws JSONException, IOException, SQLException {
		if(toBeValidated != false) {
    		...
    		alert.setContentText("No transactions to be validated");
